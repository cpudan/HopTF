...
Benign or tolerated variants are not treated as a single negative label. We separate clinical benign variants, population-tolerated variants, functional-neutral assay variants, and ortholog-supported synthetic controls. These controls are used as secondary analyses only after exact isoform mapping, hard exclusions, and balance diagnostics by TF family, domain class, DNA-binding-domain status, conservation, protein length, recovered cell count, and ESM-C-distance bin.

\placeholder{Fill in final variant curation rules after the May 17 benign/control setup. Include ClinVar review status rules, gnomAD allele-frequency and allele-count thresholds, functional-assay inclusion criteria, hard exclusion rules, and how the high-ESM-C-distance stress-test subset is reported.}
