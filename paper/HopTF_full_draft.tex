...
\subsection{Resolution-Aware Interpretation of Locus Retrieval Contexts}

The observed failure of exact-symbol JASPAR motifs and same-TF ReMap peak recovery to surpass random-rank expectations highlights a structural property of the model's bottleneck rather than a wholesale failure of feature representation. Transcription factor binding motifs typically span tight windows of 6--20 base pairs. In contrast, the addressable memory bank in \hoptf\ operates at the resolution of 1 kilobase \ag\ genomic windows. This wide window scale introduces a massive resolution mismatch: a single retrieved window contains vast non-bound flanking sequences, heavily diluting the coordinate-specific signal-to-noise ratio. 

Furthermore, exact-symbol matching ignores broad binding affinities characteristic of whole structural protein domains. TFs within the same DNA-binding domain (DBD) family—such as basic helix-loop-helix (bHLH), zinc fingers, or homeodomain proteins—frequently recognize highly overlapping consensus sequences. Because \hoptf\ utilizes continuous query representations derived from \esm\ sequence spaces, the network scores compatibility based on global protein structural semantics rather than isolated, nominal identity strings. Consequently, the retrieval layer targets general, family-level regulatory environments and highly accessible enhancer neighborhoods rather than individual, sequence-isolated docking coordinates. 

Additionally, aggregate transcriptomic changes captured by the \scarf\ response vector do not discriminate between immediate primary binding events and downstream secondary cascades. A significant fraction of the perturbation phenotype is driven by indirect regulation—where the overexpressed pioneering TF binds a localized set of master regulators, activating subsequent transcriptional networks. Under aggregate expression supervision, \hoptf\ is mathematically incentivized to maximize retrieval weights over these distal, downstream-responsive genomic windows. The retrieval distribution must therefore be framed strictly as a functional, multi-instance regulatory hypothesis tracker rather than an in silico mapping tool for direct physical protein-DNA crosslinking.
...
\caption{Variant-specific retrieval movement across \ag\ windows. Each chromosome is split into the locus bins used by HopTF. The panels show wild type, pathogenic mutant, matched benign/control sequence, and pathogenic-minus-WT change for selected examples. These are locus-window retrieval weights, not measured binding sites.}
...
\caption{Effect of retrieval inverse temperature \(\beta\). Increasing \(\beta\) concentrates retrieval mass onto fewer \ag\ windows while the identities of the highest-ranked windows remain stable across adjacent beta values. This supports the associative-retrieval interpretation but does not test beta-resolved \scarf-space prediction quality.}
...
The fact that our exact-symbol JASPAR motifs and same-TF ReMap peak recovery performed near or below random chance defines a clear limitation for the model. However, this diffuse retrieval pattern makes sense when looking at the structural constraints of our architecture and the nature of the training data.

First, there is a major resolution mismatch between standard TF binding sites and our genomic memory bank. Traditional transcription factor motifs are highly localized, usually spanning tight windows of only 6--20 base pairs. In contrast, \hoptf\ queries a memory bank partitioned into much larger 1-kilobase \ag\ windows. Because of this scale difference, the sequence-derived query ($q_p$) is not executing a rigid, base-pair-level text search for a specific DNA sequence. Instead, it behaves as a broad biometric cue that searches for an entire functional regulatory neighborhood.

Second, this window-level retrieval reflects the biophysical reality of family-level structural affinities. Transcription factors that belong to the same structural DNA-binding domain family—like bHLH or zinc-fingers—frequently recognize highly overlapping consensus sequences. Because \hoptf\ projects continuous protein representations from \esm\ into the key space, the model optimizes for general structural semantics rather than strict nominal names. This explains why the model remains highly sensitive to disruptive clinical sequences in our pathogenic variant panel, even though its intermediate retrieval weights are spread broadly across family-compatible genomic windows.

Finally, the model is trained exclusively under aggregate transcriptomic supervision using \scarf-space optimal transport. The architecture only observes the final, cell-wide expression shift induced by TF overexpression, meaning it never sees direct physical binding labels. As a result, the model is mathematically incentivized to prioritize downstream functional mediators and secondary cascading effects over primary physical docking sites. High-weight loci must therefore be interpreted strictly as model-prioritized regulatory hypotheses—representing functional, accessible neighborhoods of expression response—rather than direct, cross-linked physical binding targets.
...
\caption{Recall of exact-symbol motif and ReMap support among top-ranked retrieved windows. Recall remains near or below random-rank expectation at the current \ag-window scale, so this result should be interpreted as a limitation rather than positive binding validation.}
...
\caption{Precision-style motif and ReMap support among top-ranked retrieved windows. This diagnostic asks what fraction of retrieved windows have exact same-symbol support; it remains modest and motivates family-level, alias-aware, and accessibility-supported validation.}
